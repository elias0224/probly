...
def compute_absolute_error_score_torch(y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    ...
    pred_t = torch.as_tensor(y_pred, dtype=torch.float)
    y_true_t = torch.as_tensor(y_true, dtype=torch.float)

    if pred_t.ndim == y_true_t.ndim + 1:
        pred_t = pred_t.mean(dim=0)
    elif pred_t.ndim != y_true_t.ndim:
        msg = (
            "y_pred must match y_true shape or add a leading evaluation axis; "
            f"got y_pred shape {tuple(pred_t.shape)} and y_true shape {tuple(y_true_t.shape)}."
        )
        raise ValueError(msg)

    return torch.abs(y_true_t - pred_t)
